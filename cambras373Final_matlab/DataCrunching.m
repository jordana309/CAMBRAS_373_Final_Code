...
% Exponential model: exp1: Y = a*exp(b*x), exp2: Y = a*exp(b*x)+c*exp(d*x), in case you forgot.
...
for i=-1:50 % State Codes (plus DC-50 and All USA- -1)
  ...
  for j=1:4
    ...
    f = fit( dates', (resM(4*i+j+4,1:24))', 'exp2', 'normalize', 'off');
    c = coeffvalues(f);
    ...
    % Do other
    f = fit( dates', (othM(4*i+j+4,1:24))', 'exp2', 'normalize', 'off');
    c = coeffvalues(f);
